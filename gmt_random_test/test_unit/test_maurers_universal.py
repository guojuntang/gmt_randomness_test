...
class MaurersUniversalTest(Test):
    ...
    def _execute(self,
        ...
        # Compute magnitude
        tmp: float = (fn - self._expected_value_table[pattern_length]) / ((math.sqrt(self._variance_table[pattern_length])) * math.sqrt(2))
        magnitude: float = abs(tmp)
        # Compute the score (P-value)
        score: float = math.erfc(magnitude)
        q_value: float = math.erf(tmp) / 2.0
        # Return result
